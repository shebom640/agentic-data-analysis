...
- **Ingestion Quality Metrics:** Mathematically computes and reports two metrics to monitor data purity and pipeline operations before downstream fuzzy modeling:
  - **No. of Missing Values:** Measures dataset completeness by summing every raw empty/null cell across all dimensions in the loaded dataframe (`df.isna().sum().sum()`) before standardizing or cleaning begins.
  - **No. of Transformations:** A cumulative tracker of data manipulation operations executed to engineer the raw inputs. It increments by `1` for each column renamed/standardized; increments by `1` for each datetime string successfully parsed; adds `1` for chronological sorting; adds `1` for resetting the index; and increments by `1` for each missing/malformed cell in core logistics columns (KPT, RWT, Distance, Hour, Rating) that is successfully coerced and imputed to safe baseline values.
